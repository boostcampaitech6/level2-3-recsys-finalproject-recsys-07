...
def get_not_exist_appid_list(**kwargs):
    ...
    app_infos = pd.DataFrame(user_game_list, columns=['steamid','name','appid','playtime_forever','playtime_2weeks','img_logo_url']).astype('str')
    app_infos_newid = app_infos[~app_infos['appid'].isin(appid_pool)]
    app_list = app_infos_newid.loc[:,['appid','name']].drop_duplicates(subset='appid').values.tolist( )
    cur.close()
    conn.close()
    return app_list
...
def get_insert_and_update_list(**kwargs):
    ...
    app_infos = pd.DataFrame(user_profile_list, columns=['steamid','personaname','datatime']).astype('str')
...
def get_app_add_info(app_id):
    ...
    if response.status_code == 200:
        ...
        if item:
            ...
            appid_ = [app_id]
            combined_iter = itertools.chain(basic_data,is_adult,platforms, languages, multi,str_cate_genre,boolean_cate_genre,appid_)
...
def insert_interactions_table(**kwargs):
    ...
    user_game_list = ti.xcom_pull(task_ids='user_games_api_calls')
    app_infos = pd.DataFrame(user_game_list, columns=['steamid','name','appid','playtime_forever','playtime_2weeks','img_logo_url']).astype('str')
    interaction_list = app_infos.loc[:,['steamid','appid','playtime_forever','playtime_2weeks']].values.tolist( )
...
with DAG(
    ...
    start >> read_steamID_APIkey >> user_profile_api_calls >> user_games_api_calls >> get_not_exist_app_ids >> separation_insert_and_update_ids >> insert_user_info >> insert_interaction >> insert_app_info >> mark_app_use >> mark_user_use >> finish
